...
/// <summary>
/// Interface for managing manifest cache of shad360 emulator and shad360 Manager releases
/// </summary>
public interface IReleaseService
{
    ...
    /// <summary>
    /// Gets cached build information for a specific shad360 release type
    /// </summary>
    /// <param name="type">The type of shad360 release to retrieve</param>
    /// <returns>Cached build information or null if not available</returns>
    Task<CachedBuild?> GetCachedBuildAsync(ReleaseType type);

    /// <summary>
    /// Gets cached build information for a specific shad360 Manager release type
    /// </summary>
    /// <param name="type">The type of shad360 Manager release to retrieve</param>
...
/// <summary>
/// Service for fetching and caching manifest information about shad360 emulator and shad360 Manager releases
...
public sealed class ReleaseService : IReleaseService
{
    ...
    public async Task<CachedBuild?> GetCachedBuildAsync(ReleaseType type)
    {
        ...
        {
            ReleaseType.Shad360 => Task.FromResult(_current?.Shad360),
    ...
    public async Task<ManagerBuild?> GetManagerBuildAsync(ReleaseType type)
    {
        ...
        {
            ReleaseType.Shad360Stable => Task.FromResult(_current?.Shad360Stable),
            ReleaseType.Shad360Experimental => Task.FromResult(_current?.Shad360Experimental),
    ...
    /// Extracts information for various shad360 and shad360 Manager releases
    /// </summary>
    ...
    private static ReleaseCache BuildCache(JsonDocument doc)
    {
        ...
        // Build the cache by extracting each release type from the manifest
        ReleaseCache cache = new ReleaseCache
        {
            Shad360 = root.TryGetProperty("shad360", out JsonElement shad360Elem) ? GetBuild(shad360Elem) : null,
            Shad360Stable = root.TryGetProperty("shad360_manager_stable", out JsonElement stableElem) ? GetManagerBuild(stableElem) : null,
            Shad360Experimental = root.TryGetProperty("shad360_manager_experimental", out JsonElement expElem) ? GetManagerBuild(expElem) : null
        };

        Logger.Debug<ReleaseService>($"Built cache: Shad360={cache.Shad360?.TagName}, Stable={cache.Shad360Stable?.Version}, Experimental={cache.Shad360Experimental?.Version}");
        return cache;
    }
}