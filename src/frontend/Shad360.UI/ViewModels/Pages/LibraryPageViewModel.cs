...
using Shad360.ViewModels.Items;
...
public partial class LibraryPageViewModel : ViewModelBase
{
    ...
    [ObservableProperty] private bool _showEmulatorVersion = false;

    partial void OnShowEmulatorVersionChanged(bool value)
    {
        _settings.Settings.Ui.Window.Library.EmulatorVersion = ShowEmulatorVersion;
        _settings.SaveSettings();
    }
    ...
    private void LoadUiSettings()
    {
        ...
        ShowEmulatorVersion = librarySettings.EmulatorVersion;
    ...
    [RelayCommand]
    private void ToggleEmulatorVersion()
    {
        ShowEmulatorVersion = !ShowEmulatorVersion;
    }
    ...
    private async Task ScanDirectory()
    {
        ...
        try
        {
            List<EmulatorVersion> installedVersions = _settings.GetInstalledVersions(_settings);
            switch (installedVersions.Count)
            {
                ...
                case 1:
                    ...
                    EmulatorVersion = installedVersions[0];
                    break;
                default:
                    EmulatorVersion? chosen = await XeniaSelectionDialog.ShowAsync(installedVersions);
            ...
            (int gamesAdded, int gamesSkipped, int gamesFailed) = await AddGamesProgressDialog.ShowAsync(async (progressReporter) =>
            {
                ...
                foreach (string gameFile in discoveredGameFiles)
                {
                    ...
                    if (!details.IsValid && _settings.Settings.General.ParseGameDetailsWithXenia)
                    {
                        ...
                        details = await GameManager.GetGameDetailsWithXenia(gameFile, EmulatorVersion);
                    }
                    ...
                    try
                    {
                        ...
                        if (XboxDatabase.FilteredDatabase.Count == 1)
                        {
                            ...
                            await GameManager.AddGame(EmulatorVersion, gameInfo, gameFile, details, _settings.Settings.General.UseMediaIdForTitle);
                        }
                        else
                        {
                            // TODO: Open GameDatabaseWindow to allow the user to select the game
                            // Currently disabled
                            await GameManager.AddUnknownGame(EmulatorVersion, details, gameFile);
                        }
    ...
    {
        EmulatorVersion? EmulatorVersion = EmulatorVersion.Shad360;
        ...
        await ProcessAndAddGamesAsync(files.Select(f => f.Path.LocalPath).ToList(), Value);
    }
    ...
    /// </summary>
    private async Task<EmulatorVersion?> SelectEmulatorVersionAsync()
    {
        try
        {
            List<EmulatorVersion> installedVersions = _settings.GetInstalledVersions(_settings);
    ...
    /// </summary>
    private async Task ProcessAndAddGamesAsync(List<string> filePaths, EmulatorVersion EmulatorVersion)
    {
        ...
        foreach (string filePath in filePaths)
        {
            try
            {
                ...
                if (!details.IsValid && _settings.Settings.General.ParseGameDetailsWithXenia)
                {
                    details = await GameManager.GetGameDetailsWithXenia(filePath, EmulatorVersion);
                }
                ...
                try
                {
                    ...
                    {
                        GameInfo gameInfo = XboxDatabase.FilteredDatabase[0];
                        await GameManager.AddGame(EmulatorVersion, gameInfo, filePath, details, _settings.Settings.General.UseMediaIdForTitle);
                    }
                    else
                    {
                        await GameManager.AddUnknownGame(EmulatorVersion, details, filePath);
                    }
                }
    ...
    public async Task AddDroppedFilesAsync(List<string> filePaths)
    {
        ...
        await ProcessAndAddGamesAsync(filePaths, Value);
    }
        ...
        try
        {
            ...
            foreach (GameItemViewModel gameItem in Games)
            {
                try
                {
                    ...
                    {
                        updatedCount++;
                        updatedGames.Add($"{game.Title} ({oldRating} â†’ {game.Compatibility.Rating})");
            ...
            string message = string.Format(
                ...
                updatedCount > 0 ? string.Join("\n", updatedGames.Select(t => $"â€¢ {t}")) : "None",
                unchangedCount > 0 ? string.Join("\n", unchangedGames.Select(t => $"â€¢ {t}")) : "None",
                failedCount > 0 ? string.Join("\n", failedGames.Select(t => $"â€¢ {t}")) : "None");
    ...
    private async Task UpdateOptimizedSettings()
    {
        try
        {
            ...
            foreach (GameItemViewModel gameItem in Games)
            {
                try
                {
                    ...
                    if (!File.Exists(configPath))
                    {
                        ...
                        ConfigManager.CreateConfigurationFile(configPath, game.EmulatorVersion);
                    }
            ...
            string message = string.Format(
                ...
                updatedCount > 0 ? string.Join("\n", updatedGames.Select(t => $"â€¢ {t}")) : "None",
                unchangedCount > 0 ? string.Join("\n", unchangedGames.Select(t => $"â€¢ {t}")) : "None",
                notFoundCount > 0 ? string.Join("\n", notFoundGames.Select(t => $"â€¢ {t}")) : "None",
                failedCount > 0 ? string.Join("\n", failedGames.Select(t => $"â€¢ {t}")) : "None");
