...
namespace Shad360.ViewModels.Controls;
...
public partial class GameDetailsEditorViewModel : ObservableObject
{
    ...
    [ObservableProperty] private XeniaVersionItem _selectedEmulatorVersion;
    ...
    public GameDetailsEditorViewModel(Game game, IMessageBoxService messageBoxService)
    {
        ...
        if (installedVersions.Contains())
        {
            XeniaVersions.Add(new XeniaVersionItem(, "Canary"));
        }
        if (installedVersions.Contains())
        {
            XeniaVersions.Add(new XeniaVersionItem(, "Mousehook"));
        }
        if (installedVersions.Contains())
        {
            XeniaVersions.Add(new XeniaVersionItem(, "Netplay"));
        }

        // Always add the Custom option
        XeniaVersions.Add(new XeniaVersionItem(, "Custom"));

        // Select the current game's Xenia version (or default to first available)
        XeniaVersionItem? currentVersion = XeniaVersions.FirstOrDefault(v => v.Version == game.EmulatorVersion);
        SelectedEmulatorVersion = currentVersion ?? XeniaVersions.First();
    ...
    public async Task<bool> SaveAsync()
    {
        ...
        try
        {
            ...
            if (filteredTitle != _game.Title)
            {
                ...
                string newConfigPath = Path.Combine(EmulatorPaths.ConfigFolderLocation, $"{filteredTitle}.config.toml");
                ...
                if (_game.FileLocations.Patch != null)
                {
                    ...
                    string newPatchPath = Path.Combine(EmulatorPaths.ConfigFolderLocation, $"{_game.GameId} - {filteredTitle}.patch.toml");
            ...
            EmulatorVersion newVersion = SelectedVersion;
            if (newVersion != _game.EmulatorVersion && newVersion != )
            {
                Logger.Info<GameDetailsEditorViewModel>($"Xenia version changed from '{_game.EmulatorVersion}' to '{newVersion}'");

                // Get the new emulator paths
                XeniaVersionInfo newVersionInfo = EmulatorPaths;
            ...
            _game.EmulatorVersion = SelectedVersion;
            _game.FileLocations.CustomEmulatorExecutable = SelectedVersion ==  ? CustomExecutablePath : null;
    ...
    private void Cancel()
    {
        ...
        XeniaVersionItem? currentVersion = XeniaVersions.FirstOrDefault(v => v.Version == _game.EmulatorVersion);
        SelectedEmulatorVersion = currentVersion ?? XeniaVersions.First();
...
public record XeniaVersionItem(EmulatorVersion Version, string DisplayName);
