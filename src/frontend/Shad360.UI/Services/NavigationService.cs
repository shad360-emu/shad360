...
using Shad360.Views.Pages;

namespace Shad360.Services;
...
public class NavigationService
{
    ...
    public async Task NavigateToTag(string tag, Frame? contentFrame = null)
    {
        ...
        bool isShad360Installed = settings.IsShad360Installed(settings);

        switch (tag)
        {
            case "Open":
                Logger.Info<NavigationService>("Processing 'Open' tag - attempting to launch shad360");
                try
                {
                    if (!isShad360Installed)
                    {
                        Logger.Error<NavigationService>("shad360 not installed");
                    ...
                    Logger.Info<NavigationService>("shad360 is installed, launching directly");
                    EventManager.Instance.DisableWindow();
                    await Launcher.LaunchEmulatorAsync(EmulatorVersion.Shad360);
            ...
            case "Library":
                ...
                break;
            case "Shad360Settings":
                Logger.Debug<NavigationService>("Navigating to shad360 Settings page");
                if (!isShad360Installed)
                {
                    Logger.Warning<NavigationService>("shad360 not installed, navigating to Manage page instead");
                    frame.Navigate(typeof(ManagePage), null, new EntranceNavigationTransitionInfo());
                }
                else
                {
                    frame.Navigate(typeof(Shad360SettingsPage), null, new EntranceNavigationTransitionInfo());
                }
                break;
            case "Manage":
