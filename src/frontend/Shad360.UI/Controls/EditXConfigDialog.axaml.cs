...
public partial class EditXConfigDialog : UserControl
{
    ...
    /// <param name="EmulatorVersion">The Xenia version to use for saving the XConfig.</param>
    /// <returns>True if the user saved changes, false if the user canceled the dialog.</returns>
    public static async Task<bool> ShowAsync(XConfigFile xconfig, List<AccountInfo> profiles, EmulatorVersion EmulatorVersion)
    {
        Logger.Info<EditXConfigDialog>($"Showing Edit XConfig dialog for {EmulatorVersion}");
