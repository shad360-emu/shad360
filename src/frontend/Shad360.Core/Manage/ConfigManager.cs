...
public class ConfigManager
{
    ...
    /// </summary>
    private static readonly Dictionary<EmulatorVersion, (string DefaultConfigLocation, string ConfigLocation)> _configLocations = new Dictionary<EmulatorVersion, (string DefaultConfigLocation, string ConfigLocation)>
    {
        {
            ,
    ...
    /// <param name="EmulatorVersion">The Xenia version for which to apply the configuration</param>
    ...
    public static bool ChangeConfigurationFile(string configurationFile, EmulatorVersion EmulatorVersion)
    {
        // Attempt to retrieve the configuration paths for the specified Xenia version
        if (!_configLocations.TryGetValue(EmulatorVersion, out (string DefaultConfigLocation, string ConfigLocation) configPaths))
        {
            // Throw exception if the specified Xenia version is not supported
            throw new NotImplementedException($"Unsupported emulator version: {EmulatorVersion}");
        }
    ...
    /// </para>
    /// </summary>
    /// <param name="EmulatorVersion">Xenia Version we're generating the configuration file</param>
    ...
    /// </remarks>
    public static void GenerateEmulatorConfigurationFile(EmulatorVersion EmulatorVersion, bool generateProfile = false)
    {
        // Fetch XeniaVersionInfo
        XeniaVersionInfo info = EmulatorPaths;
    ...
    /// <param name="EmulatorVersion">The Xenia version for which to create the configuration file</param>
    ...
    public static bool CreateConfigurationFile(string configurationFile, EmulatorVersion EmulatorVersion)
    {
        Logger.Trace<ConfigManager>($"Starting CreateConfigurationFile operation for Xenia version: {EmulatorVersion}");
        ...
        try
        {
            ...
            if (!File.Exists(defaultConfigPath))
            {
                ...
                GenerateEmulatorConfigurationFile(EmulatorVersion);
                if (!File.Exists(defaultConfigPath))
                {
                    throw new Exception($"Couldn't find & generate default configuration file for Xenia {}");
                }
            }
            ...
            Logger.Debug<ConfigManager>($"CreateConfigurationFile operation completed successfully for Xenia version: {EmulatorVersion}");
            return true;
        }
        catch (Exception ex)
        {
    ...
    /// <param name="EmulatorVersion">The Xenia version whose configuration needs to be saved</param>
    /// <exception cref="NotImplementedException">Thrown when the specified Xenia version is not supported</exception>
    public static void SaveConfigurationFile(string configurationFile, EmulatorVersion EmulatorVersion)
    {
        Logger.Debug<ConfigManager>($"Starting to save configuration file for Xenia version: {EmulatorVersion}");
        ...
        if (!_configLocations.TryGetValue(EmulatorVersion, out (string DefaultConfigLocation, string ConfigLocation) configPaths))
        {
            Logger.Error<ConfigManager>($"Unsupported emulator version requested: {EmulatorVersion}");
            throw new NotImplementedException($"Unsupported emulator version: {EmulatorVersion}");
        }
        ...
        Logger.Debug<ConfigManager>($"Finished saving configuration file for Xenia version: {EmulatorVersion}");
    }
    ...
    /// <param name="EmulatorVersion">The Xenia version for which to create the new config.</param>
    ...
    public static bool MigrateConfigurationFile(string oldConfigPath, string newConfigPath, EmulatorVersion EmulatorVersion)
    {
        ...
        try
        {
            ...
            CreateConfigurationFile(AppPathResolver.GetFullPath(newConfigPath), EmulatorVersion);
