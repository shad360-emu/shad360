...
using Shad360.Core.Models.Database.Patches;
using Shad360.Core.Models.Game;
using Shad360.Core.Models.Files.Patches;
...
public class PatchManager
{
    ...
    public static async Task DownloadPatchAsync(Game game, PatchInfo patch)
    {
        ...
        Logger.Debug<PatchManager>($"Game Xenia version: {game.EmulatorVersion}, Patch download URL: {patch.DownloadUrl}");

        // Get the patch folder location for the game's Xenia version
        string patchFolderLocation = EmulatorPaths.PatchFolderLocation;
        Logger.Debug<PatchManager>($"Patch folder location for {game.EmulatorVersion}: {patchFolderLocation}");
    ...
    public static async Task InstallLocalPatchAsync(Game game, PatchFile patchFile, string patchFileLocation, Func<string, string, string, Task<bool>>? confirmMismatch = null)
    {
        ...
        await Task.Run(() =>
        {
            try
            {
                ...
                string destinationLocation = Path.Combine(EmulatorPaths.PatchFolderLocation, destinationFileName);
    ...
    /// <param name="EmulatorVersion">The Xenia version to get the patch folder location.</param>
    /// <returns>A task representing the asynchronous operation.</returns>
    public static async Task DisablePatchesAsync(Game game, EmulatorVersion EmulatorVersion)
    {
        await Task.Run(() =>
        {
            try
    ...
    /// <summary>
    /// Disables patch files that match the game's ID or alternative IDs synchronously.
    /// </summary>
    /// <param name="game">The game object containing game ID and alternative IDs.</param>
    /// <param name="EmulatorVersion">The Xenia version to get the patch folder location.</param>
    public static void DisablePatches(Game game, EmulatorVersion EmulatorVersion)
    {
        try
        {
            Logger.Info<PatchManager>($"Starting patch disabling process for game: '{game.Title}' (ID: {game.GameId})");
    ...
    /// <summary>
    /// Restores all previously disabled patch files by removing the .disabled extension synchronously.
    /// </summary>
