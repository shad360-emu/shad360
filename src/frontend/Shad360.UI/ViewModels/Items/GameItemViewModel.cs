...
using Shad360.Core.Models.Files.Bindings;
using Shad360.Core.Models.Files.Config;
using Shad360.Core.Models.Game;
using Shad360.Core.Models.Items;
...
public partial class GameItemViewModel : ViewModelBase
{
    ...
    public bool IsCustomXenia => Game.EmulatorVersion == ;
    public bool IsXeniaMousehook => Game.EmulatorVersion == ;
    ...
    private async Task ViewInstalledContent()
    {
        try
        {
            ...
            [
                new GameContent(Game.EmulatorVersion, Game.GameId) // Game Content (universal content with XUID 0)
            ];

            // Add account-specific content
            List<AccountInfo> accountInfos = ProfileManager.LoadProfiles(Game.EmulatorVersion);
            foreach (AccountInfo info in accountInfos)
            {
                accountContents.Add(new AccountContent(info, Game.EmulatorVersion, Game.GameId));
            }
    ...
    private async Task ViewScreenshots()
    {
        try
        {
            string screenshotsFolder = AppPathResolver.GetFullPath(EmulatorPaths.EmulatorDir,
    ...
    private async Task EditMousehookControls()
    {
        try
        {
            string? bindingsLocation = EmulatorPaths.BindingsLocation;
