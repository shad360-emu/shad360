...
public partial class ManageProfilesDialog : UserControl
{
    ...
    /// <param name="EmulatorVersion">The Xenia version to use for profile management.</param>
    /// <returns>True if the user saved changes, false if the user canceled the dialog.</returns>
    public static async Task<bool> ShowAsync(List<AccountInfo> profiles, EmulatorVersion EmulatorVersion = )
    {
        ...
        dialog._viewModel.LoadProfiles(profiles, EmulatorVersion);
        ...
        contentDialog.PrimaryButtonClick += async (_, e) =>
        {
            ...
            try
            {
                ...
                int savedCount = await Task.Run(() => ProfileManager.SaveProfiles(dialog._viewModel.Profiles, dialog._viewModel.EmulatorVersion));
