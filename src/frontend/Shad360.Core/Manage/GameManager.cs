...
using Shad360.Core.Models.Database.Xbox;
using Shad360.Core.Models.Files;
...
public class GameManager
{
    ...
    public static void SortLibrary(GameSortOption sortOption = GameSortOption.Title, bool descending = false)
    {
        ...
        Comparison<Game> comparison = sortOption switch
        {
            ...
            GameSortOption.EmulatorVersion => (x, y) => x.CompareTo(y.EmulatorVersion),
    ...
    public static async Task<ParsedGameDetails> GetGameDetailsWithXenia(string gamePath, EmulatorVersion version)
    {
    ...
    /// </summary>
    /// <param name="EmulatorVersion">The Xenia version to associate with this game.</param>
    ...
    public static async Task AddGame(EmulatorVersion EmulatorVersion, GameInfo selectedGame, string gamePath, ParsedGameDetails details, bool useMediaIdForTitle = false)
    {
        ...
        Logger.Trace<GameManager>($"Starting AddGame operation - TitleId: {actualTitleId}, MediaId: {details.MediaId}, EmulatorVersion: {EmulatorVersion}, GamePath: {gamePath}");
        ...
        newGame.FileLocations.Config = Path.Combine(EmulatorPaths.ConfigFolderLocation, $"{newGame.Title}.config.toml");
        Logger.Debug<GameManager>($"Configuration file path: {newGame.FileLocations.Config}");
        ConfigManager.CreateConfigurationFile(AppPathResolver.GetFullPath(newGame.FileLocations.Config), EmulatorVersion);
        ...
        if (!File.Exists(boxartSavePath))
        {
            ...
            ArtworkManager.LocalArtwork("Shad360.Core.Assets.Artwork.Boxart.jpg", boxartSavePath, SKEncodedImageFormat.Png);
        ...
        if (!File.Exists(iconSavePath))
        {
            ...
            ArtworkManager.LocalArtworkAsIcon("Shad360.Core.Assets.Artwork.Icon.png", iconSavePath);
        ...
        if (!File.Exists(backgroundSavePath))
        {
            ...
            ArtworkManager.LocalArtwork("Shad360.Core.Assets.Artwork.Background.jpg", backgroundSavePath, SKEncodedImageFormat.Jpeg);
    ...
    /// <param name="gamePath">The file path to the game.</param>
    /// <returns>A task representing the asynchronous operation.</returns>
    public static async Task AddUnknownGame(EmulatorVersion EmulatorVersion, ParsedGameDetails details, string gamePath)
    {
        Logger.Trace<GameManager>($"Starting AddUnknownGame operation - Title: '{details.Title}', TitleId: {details.TitleId}, MediaId: {details.MediaId}, EmulatorVersion: {EmulatorVersion}, GamePath: {gamePath}");
        ...
        ArtworkManager.LocalArtwork("Shad360.Core.Assets.Artwork.Boxart.jpg",
        ...
        ArtworkManager.LocalArtworkAsIcon("Shad360.Core.Assets.Artwork.Icon.png",
        ...
        ArtworkManager.LocalArtwork("Shad360.Core.Assets.Artwork.Background.jpg",
    ...
    public static void RemoveGame(Game game, bool deleteGameContent = false)
    {
        ...
        if (deleteGameContent)
        {
            ...
            string emulatorContentFolder = AppPathResolver.GetFullPath(EmulatorPaths.ContentFolderLocation);
